...
class Heatmap(ObjectCounter):
    ...
    def __init__(self, **kwargs):
        ...
        # show original image with heatmap
        self.show = self.CFG["show"] if "show" in self.CFG else False
    ...
    def generate_heatmap(self, im0):
        ...
        # Normalize, apply colormap to heatmap and combine with original image for display
        heatmapColorMap = cv2.applyColorMap(
                        cv2.normalize(self.heatmap, None, 0, 150, cv2.NORM_MINMAX).astype(np.uint8), self.colormap
                    )
        im0 = (
            cv2.addWeighted(
                im0,
